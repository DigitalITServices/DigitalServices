  ...
  <div class="relative">
    ...
    <div class="mx-auto mt-32 max-w-7xl px-6 sm:mt-20 lg:px-8">
      ...
      <div class="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
        <article v-for="post in posts" :key="post.path" class="flex flex-col items-start justify-between bg-orange-50">
          ...
          <div class="flex max-w-xl grow flex-col justify-between">
            <div class="mt-8 flex items-center gap-x-4 text-xs">
              <time :datetime="post.datetime" class="text-gray-900">{{ post.date }}</time>
            ...
            <div class="group relative grow">
              <h3 class="mt-3 text-lg/6 group-hover:text-green-600">
                <NuxtLink :to="post.path">
                  <span class="absolute inset-0"></span>
                  {{ post.title }}
                </NuxtLink>
              </h3>
              <p class="mt-5 line-clamp-3 text-sm/6">{{ post.description }}</p>
            </div>
