<template>
  <nav class="flex bg-slate-50" aria-label="Breadcrumb">
    <ol role="list" class="flex space-x-4 rounded-md bg-slate-50 px-6 shadow-sm dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10">
      <li class="flex">
      ...
      <li v-for="page in pages" :key="page.name" class="flex">
        <div class="flex items-center">
          ...
          <a :href="page.href" class="ml-4 text-sm font-medium text-green-600 hover:text-emerald-500 dark:text-green-600 dark:hover:text-emerald-500" :aria-current="page.current ? 'page' : undefined">{{ page.name }}</a>
        </div>
      </li>
    </ol>
  </nav>
